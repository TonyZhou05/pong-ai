...
- `clips/` — the fixture corpus. `synthetic_demo.json` carries a scoring ground
  truth only (Stage 1); `synthetic_labeled.json` additionally carries
  `groundTruthFrames` + `groundTruthEvents` so the shipped corpus also exercises
  Stage 2 (perception) and Stage 3 (event detection). It is (re)generated by
  `dart run tool/gen_labeled_clip.dart`, which builds the clip and verifies the
  declared score/events against the real pipeline before writing the JSON.
